...
Bool sample_rate_change = FALSE; // playback sample rate change flag
Bool rec_sample_rate_change = FALSE; // record sample rate change flag -- NOTE no record sample rate control
...
//Uint16 numPbSamps; // number of samples to playback
Uint16 numRecSamps; // number of samples to record
...
CSL_Status AC_Open(pAcAppClassHandle    pAppClassHandle)
{
    ...
    if(pAppClassHandle != NULL)
    {
        ...
        pHandle->ctrlHandle.hEpObjArray[5] = NULL;
        ...
        pHandle->ctrlHandle.sampleRateBuf[1] = 0xBB80; /* playback sampling rate = 48 kHz */
        pHandle->ctrlHandle.sampleRateBuf[2] = 0x0000;

        pHandle->ctrlHandle.recSampleRateBuf[0] = 0x0000;
        pHandle->ctrlHandle.recSampleRateBuf[1] = 0x3E80; /* record sampling rate = 16 kHz */
        pHandle->ctrlHandle.recSampleRateBuf[2] = 0x0000;
        ...
        /* Initialize the Iso Endpoint OUT 2 */
        USB_initEndptObj(pHandle->ctrlHandle.devNum,
        ...
        /* Initialize the Iso Endpoint IN 1 */
        USB_initEndptObj(pHandle->ctrlHandle.devNum,
        ...
        /* Initialize the HID Interrupt Endpoint IN 3 */
        USB_initEndptObj(pHandle->ctrlHandle.devNum,
...
CSL_Status AC_Iso(void    *pAcObj)
{
    ...
    if(pAcObj != NULL)
    {
        ...
        if((usbEvent & CSL_USB_EVENT_READ_MEDIA) == CSL_USB_EVENT_READ_MEDIA)
        {
            ...
            {
                /* get record data */
                status = pAcHandle->recordAudio(numRecSamps, 
                    &pAcHandle->lbaBufferRec[0], &pAcHandle->lbaBufferRec[0], 
                    (void*)(&pAcHandle->recordActive));

                USB_postTransaction(&pAcHandle->isoInEpObj, 
                    2*numRecSamps, (void *)(&pAcHandle->lbaBufferRec[0]), 
                    CSL_USB_IOFLAG_NONE);
            }
        }
                                    ...
                                    void                  *pAcObj)
{
    ...
    if ((usbSetup->bmRequestType & 0x0F) == 0x1) /* request sent to (audio control) interface */
    {
        if ((usbSetup->wIndex >> CSL_AC_8BIT_SHIFT) == 0x2) /* FU ID=2 => playback */
        {
            switch (usbSetup->wValue)
            {
                ...
                case ((USB_AC_FEATURE_UNIT_VOLUME_CTRL << 8) | USB_AC_FEATURE_UNIT_LEFT_VOLUME): 
                ...
                case ((USB_AC_FEATURE_UNIT_VOLUME_CTRL << 8) | USB_AC_FEATURE_UNIT_RIGHT_VOLUME): 
                ...
                case ((USB_AC_FEATURE_UNIT_VOLUME_CTRL << 8) | USB_AC_FEATURE_UNIT_LEFT_RIGHT_VOLUME):
    ...
    {
        if (usbSetup->wIndex == CSL_USB_EP2) // is data for playback endpoint
        {
            sample_rate_change = TRUE;
        ...
        else if (usbSetup->wIndex == CSL_USB_EP1) // is data for record endpoint
        {
            rec_sample_rate_change = TRUE;
                                    ...
                                    void                  *pAcObj)
{
    ...
    else if (usbSetup->wIndex == 1) /* host enable and disable record audio path */
    {
        if (usbSetup->wValue == 0)
        {
            if (pAcHandle->recordActive == TRUE)
    ...
    else if (usbSetup->wIndex == 2)      /* host enable and disable record playback path */
    {
        if (usbSetup->wValue == 0)
        {
